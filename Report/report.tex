...
\begin{enumerate}
  ...
  Note that we need a decaying $\epsilon$ because we want to explore more in the beginning, and as we approach the end of the training phase, we want to exploit more.\footnote{This gives two extra hyperparameters $\epsilon$ and $\delta$ that need to be adjusted.}
...
\begin{itemize}
  \item 
  For the handcrafted agent, we will return a list of 4 feature values
  \item 
  For the local search agent, we will return a list of 10 feature values
  \item 
  For the reinforcement learning agent, we will return a list of $20\times 10$ bitmaps representing all the successor states
\end{itemize}
Finally, the agent will return its decision to the model, and it will update the current state correspondingly.
...
\begin{itemize}
  ...
  Now, recall that we introduced the special technique \textit{training by part} in the \textbf{Methodology} section and here is the detailed explanation:
