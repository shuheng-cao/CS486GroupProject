...
For the genetic algorithm, we have the following contributions
\begin{itemize}
    \item 
    
    We implemented genetic algorithm for the local search agent
    
    \item
    
    We found a positive relationship between number of games played and the scores achieved
    
    \item
    
    Our local search agent achieved best performance for the game of Tetris
\end{itemize}
...
we will focus on discussion on the local search agent as well as the reinforcement learning agent.

First, we will discuss some interpretation, implication and limitation of our local search agent approach. 

\begin{itemize}
    \item \textbf{Interpretation}
    
    One of the main purposes of our paper was to compare and contrast different methodologies in solving Tetris game. With our results, we are surprised by how well our local search agent performs in the game. At first, we expected the local search agent to improve upon the handcrafted agent by automatically finding best weights instead of a manually trial-and-error approach. We did not expect it to perform very well since this approach still requires manually defining features which we consider would impair the performance. We expected that the reinforcement learning agent will have the best performance. One significance of this unexpected result is that the performance of Tetris game depends more heavily on the search of optimal weights of features, making it more effective for a search-algorithm-based approach to excel. Another significance is that while reinforcement learning is powerful and we have assumed it would be an optimal approach, it has some limitations in the Tetris game setting. We will discuss more about this limitation later. 
    
    \item \textbf{Implication}
    
    Since local search agent approach is not our entire focus in this paper, and that there is scarce reference of analysing local search approach's performance against a Tetris game, the scale, design, implementation and evaluation of our approach have some fundamental differences from previous works. Nevertheless, our result agrees with previous researches. That is, with sufficient time of searching for optimal hyper-parameter values, a local search agent can last very long in the game and achieve very high scores.
    
    \item \textbf{Limitation}
    
    Even though we have surprisingly good results from our local search agent approach, there are still some limitations to it.
    
    Firstly, we might not have found the optimal hyper-parameter values for our local search agent simply due to lack of computation power. In total, we only tested 24 combinations over 10 features. We know we should expand our search space and we acknowledge that there are probably more relevant features that we should take into consideration. However, we were not able to make the experiment more comprehensive given our hardware capabilities.
    
    Secondly, although we deliberately compare local search algorithms and reinforcement learning algorithms in our research and reached an conclusion that the local search algorithms performs better than its counterpart. It does not mean the former is absolutely better than the latter in the Tetris game. Specifically, we used genetic algorithm as the main driver in the local search agent and DQN in the reinforcement learning agent. We acknowledge the fact that there may be other implementations of both agents that would result in different game-plays and different conclusions. In other words, our result does not prove that a local search agent will always perform better than a reinforcement learning agent in the game of Tetris.
\end{itemize}
...
\end{table}  
  ...
  \begin{figure}[h!]
    ...
    \includegraphics[width=0.185\textwidth]{figures/example1.png}
    }\hfill
    \subfigure[score=2.34137]{
    \includegraphics[width=0.185\textwidth]{figures/example2.png}
    }
    \subfigure[score=1.13586]{
    \includegraphics[width=0.185\textwidth]{figures/example3.png}
    }\hfill
    \subfigure[score=0.96125]{
    \includegraphics[width=0.185\textwidth]{figures/example4.png}
    }
    \subfigure[score=-0.80523]{
    \includegraphics[width=0.185\textwidth]{figures/example5.png}
    }\hfill
    \subfigure[score=-0.79563]{
    \includegraphics[width=0.185\textwidth]{figures/example6.png}
    }
